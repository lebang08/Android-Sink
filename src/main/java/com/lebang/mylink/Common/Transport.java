...
package com.lebang.mylink.Common;
...
import com.lebang.mylink.YXApplication;
    ...
    final class ReaderThread extends Thread {
        ...
        private void loop() {
            ...
            outer: while (!mQuitting) {
                ...
                int count;
                try {
//                    Toast.makeText(YXApplication.getContext(),buffer.array().toString(),Toast.LENGTH_LONG).show();//TODO LeBang
                    count = ioRead(buffer.array(), position, buffer.capacity() - position);
//                    getLogger().log("LeBang  buffer.array() = " + count + " , " +  Arrays.toString(buffer.array()));
                    if (count < 0) {
                        break; // end of stream
                    }
                } catch (IOException ex) {
